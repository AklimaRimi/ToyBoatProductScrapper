...
#     lists=driver.find_element(By.ID,'detail-features').text
#     description += '\n\n'+lists
...
df = pd.read_csv('C:/pythons/Deep Learning/tasks/task2/ToyBoatProductScrapper/output/info.csv')
def rep(x):
    x = x.replace('[','')
    y = x.replace(']','')
    z = y.replace("'",'')
...
print(df.head())
df.to_csv('C:/pythons/Deep Learning/tasks/task2/ToyBoatProductScrapper/output/info.csv',index = False)
# print(len(imgs))
